...
class FewshotClothDataset(BaseDataset):
    @staticmethod
    def modify_commandline_options(parser, is_train):
        parser.set_defaults(dataroot='datasets/cloth/')
    ...
    def __getitem__(self, index):
        ...
        if opt.isTrain:
            ...
            ref_img_paths, ref_lbl_paths = img_paths, lbl_paths
        ...
        is_first_frame = opt.isTrain or index == 0# index=3463 
        ...
        if is_first_frame: # need to read reference images for every training iter or at beginning of inference            
            ...
            for i, idx in enumerate(ref_indices):                
                ...
                ### my changed
                Li, Ii, ref_crop_coords[i], ref_face_pts = self.get_images( # error
                    ref_img_paths, ref_lbl_paths,#ref_op_paths, ref_dp_paths, ### my changed
        ...
        for t in range(n_frames_total):
            ...
            Lt, It, crop_coords, _ = self.get_images( # error ?crop_coords cant None?
                img_paths, lbl_paths, #op_paths, dp_paths, ### my changed
                idx, size, img_params, 
    ...
    def get_images(self, img_paths, lbl_paths, i, size, params, crop_coords, ref_face_pts=None):
        ...
        return Li, Ii, crop_coords, None#face_pts ### my changed
    
    def get_image(self, A_path, size, params, crop_coords, input_type, ppl_idx=None, op=None, ref_face_pts=None, is_label=False):
        if A_path is None: return None, None
        opt = self.opt        
        is_img = input_type == 'img'
        is_lbl = input_type == 'lbl'
        ...
        A_img = self.read_data(A_path, data_type='img')            
        '''
        A_img, _ = self.crop_person_region(A_img, crop_coords)# ) ### my changed error ?crop_coords cant None?
        if input_type == 'densepose':  # remove other ppl in the densepose map
            A_img = self.remove_other_ppl(A_img, A_path, crop_coords, op)'''
        ### my created
        if is_lbl or A_img.mode in 'RGBA':
            A_img = A_img.convert('RGB') # normalize in transform_scaleA cant imput RGBA image
    ...
    def crop_person_region(self, A_img, crop_coords, pose_pts=None, size=None):
        ...
        if crop_coords is None: 
            ...
            crop_coords = self.get_crop_coords(pose_pts, size, random_offset) # error size cant None
    ...
    def name(self):
        return 'ClothDataset'#'PoseDataset'